// BGAPIを使用する場合、基本的にいじる必要があるのはこのファイルだけです。
...
namespace BGAPI
{
	extern volatile bool cmd_success;
	extern volatile bool connected;
}

namespace GUI
{
	extern volatile HWND main_dlg;
}


// 以下、メッセージに応じたイベントハンドラに必要な処理を記述
void ble_default(const void* nil)
{
}

void ble_rsp_hardware_set_rxgain(const void* nil)
{
}

void ble_rsp_system_aes_setkey(const void* nil)
{
}
...
void ble_rsp_test_channel_mode(const void* nil)
{
}
...
void ble_evt_system_no_license_key(const void* nil)
{
}
...
void ble_rsp_hardware_set_txpower(const void* nil)
{
}
...
void ble_rsp_attributes_user_read_response(const void* nil)
{
}

void ble_rsp_attributes_user_write_response(const void* nil)
{
}

void ble_rsp_system_whitelist_clear(const void* nil)
{
}
...
void ble_rsp_attclient_reserved(const void* nil)
{
}
...
void ble_rsp_sm_set_parameters(const void* nil)
{
}
...
void ble_rsp_sm_set_oob_data(const void* nil)
{
}
...
void ble_rsp_sm_set_security_mode(const void* nil)
{
}
...
void ble_rsp_hardware_analog_comparator_enable(const void* nil)
{
}
...
void ble_rsp_gap_set_privacy_flags(const void* nil)
{
}
...
void ble_rsp_system_reset(const void* nil)
{
}

void ble_rsp_system_hello(const void* nil)
{
}
...
void ble_rsp_flash_ps_defrag(const void* nil)
{
}

void ble_rsp_flash_ps_dump(const void* nil)
{
}

void ble_rsp_flash_ps_erase_all(const void* nil)
{
}
...
void ble_rsp_flash_ps_erase(const void* nil)
{
}
...
{
	OutputDebugString("<<< ble_rsp_connection_disconnect\n");
}
...
{
	OutputDebugString("<<< ble_rsp_attclient_attribute_write\n");

	BGAPI::cmd_success = (msg->result == 0);
}
...
void ble_rsp_sm_set_bondable_mode(const void* nil)
{
}
...
void ble_rsp_gap_set_address_mode(const void* nil)
{
}
...
void ble_rsp_test_phy_tx(const void* nil)
{
}

void ble_rsp_test_phy_rx(const void* nil)
{
}
...
void ble_rsp_test_phy_reset(const void* nil)
{
}
...
{
	OutputDebugString("### ble_evt_connection_status\n");
	...
	if (msg->flags & connection_connected)
	{
		...
		BGAPI::connected = true;
		MessageBox( NULL, "PLEN2との接続に成功しました。", "ble_evt_connection_status()", MB_OK);
	}
}
...
void ble_evt_gap_scan_response(const struct ble_msg_gap_scan_response_evt_t* msg)
{
	...
	OutputDebugString("### ble_evt_gap_scan_response\n");

	// データパケットの長さが25以上であれば、UUIDが乗っていないかチェックする。
	// ========================================================================
	// CAUTION!：
	// このUUIDチェックは、アドバタイズパケットの仕様変更があった場合修正が必要です。
	if (msg->data.len > 25)
	{
		// 10byte目から16byte分がUUIDと定義している。(iBeaconの実装を参考にした。)
		char data_buf[16];
		...
		if (memcmp(data_buf, PLEN2_TX_CHARACTERISTIC_UUID, UUID_LENGTH) == 0)
		{
			...
			{
				mac << std::setfill('0') << std::setw(2) << std::hex << static_cast<int>(msg->sender.addr[index]);
			}

			SetDlgItemText(GUI::main_dlg, EDIT_MAC, mac.str().c_str());
	...
	else
	{
		...
		// 以下は横着実装、本当はちゃんとキャラクタリスティックハンドルを調べないとダメ
		std::stringstream mac;
		for (int index = 0; index < 6; index++)
		{
			mac << std::setfill('0') << std::setw(2) << std::hex << static_cast<int>(msg->sender.addr[index]);
...
void ble_rsp_dfu_reset(const void* nil)
{
}
